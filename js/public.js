...
function chkallchecked(obj) {
    var chlist = document.getElementsByTagName("input");
    var count = chlist.length;
    for (var i = 0; i < count; i++) {
        if (chlist[i].type == "checkbox") {
            chlist[i].checked = obj.checked;
        }
    }
}

function clearLaber(id) {
    setTimeout(function () { $("#" + id).html(""); $("#MsgBox").css("display", "none"); }, 3000);
}

function MsgErr(str) {

    $("#MsgBox").html("<div class='error'>" + str + "</div>");
    $("#MsgBox").css("display", "inline");
    clearLaber("MsgBox");
}

function MsgSucc(str) {

    $("#MsgBox").html("<div class='success'>" + str + "</div>");
    $("#MsgBox").css("display", "inline");
    clearLaber("MsgBox");
}
...
function Msg2(text, top, left, refurl) {

    var content = ' <div id="ChangeDialog" style="position: absolute; top: ' + top + 'px; left: ' + left + 'px; z-index: 999;width: 339px; display: none;height: 155px; background: url(/images/platform/duidui_1.jpg) no-repeat;">';
    content += '  <div class="duidui_cpdh_header"><div class="gb"><a href="javascript:void(0);" onclick=\'CloseDialog( "' + refurl + '");\'></a></div> </div>';
    content += '  <div class="duidui_nr"> <p id="tipMsg"  >' + text + ' </p> </div>';
    content += '<div id="submitDiv" style="height:35px; width:100px; margin:0px auto;"><input type="submit" name="button" id="button1" value="" onclick=\'CloseDialog( "' + refurl + '");\' /> </div> </div>';
    $("body").append(content);

}
function CloseDialog(refurl) {
...
function OpenDialog(text, top, left, refurl) {

    Msg2(text, top, left, refurl);
...
function GetRequestgg(paramName, defaultValue) {
...
function CheckLogin() {
    ...
    $.get(isLogin, function (data) {
        if (data != "0") {
            ...
            var logined = "";
            logined += "<ul>";
            logined += "    <li style=\"color: #fff; width: 300px; float: left;\">";
            logined += "        <table>";
            logined += "            <tr>";
            logined += "                <td>" + str[0] + ",欢迎您!</td>";
            logined += "                <td><a href=\"\/Members/\" style=\"width: 50px; float: left;\">用户中心</a></td>";
            logined += "                <td><a href=\"javascript:void(0)\" onclick=\"ExitLogin();\">退出</a></td>";
            logined += "            </tr>";
            logined += "        </table>";
            logined += "    </li>";
            logined += "</ul>";

            $("#topreglogin").html(logined);

            $("#user_nologin").hide();
            $("#user_logined").show();

            $("#user_name").html(str[0]);
            $("#wallet_money").html(str[1]);
            $("#bank_money").html(str[2]);
            $("#user_voucher").html(str[3]);
        }
        else {
            var loginStr = "";
            loginStr += "<ul>";
            loginStr += "   <li class=\"tzy_dd\"><a href=\"Login.aspx\">登录</a></li>";
            loginStr += "   <li class=\"tzy_dd\"><a href=\"Register.aspx\">注册</a></li>";
            loginStr += "   <li class=\"tzy_dd\"><a href=\"#\">设为首页</a></li>";
            loginStr += "   <li><a href=\"#\">添加到收藏夹</a></li>";
            loginStr += "   <li class=\"tzy_dd\"></li>";
            loginStr += "</ul>";
            $("#topreglogin").html(loginStr);
            $("#user_nologin").show();
            $("#user_logined").hide();
...
function ExitLogin() {
    ...
    if (confirm("确认退出登录吗？")) { 
      var url = "/Members/MembersHandler.ashx?action=exitlogin&t=" + Math.random();
